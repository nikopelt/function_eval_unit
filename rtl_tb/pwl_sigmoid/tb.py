...
"""
    Testbench to test the functionality of a sigmoid unit for the integration to a multi-function unit of an LLM accelerator
"""
def sigmoid(x):
    return 1/(1+np.exp(-x))
...
async def testbench(dut):
    ...
    cocotb.log.info("Testbench for the testing of the approximation of a function with fixed point input")
    ...
    cocotb.log.info("Output type : %s", type(dut.sigma_x.value))
    ...
    cocotb.log.info("Hex to Float : %f", hex_fixed_to_float(dut.sigma_x.value, 8, 24))
    ...
    for i in range(800):
        ...
        y_points.append(hex_fixed_to_float(dut.sigma_x.value))
        
        input_val += 0.0125

    actual_points = sigmoid(np.array(points))
